...
export default class UserService extends BaseService<User> {
   ...
   * @param opts
   */
  async updateUser(data: DeepPartial<User>, where: any) {
  ...
  async checkUser(userId: string) {
    const {
      ...
      NO_PERMISSION_UPDATE_SYSTEM_USER_ERROR,
      NO_PERMISSION_UPDATE_OTHER_PLATFORM_USER_ERROR,
    } = this.ctx.app.exception.user;
    const {
      userData: { userinfo },
    } = this.ctx;
    ...
    // 判断修改的人员权限是否大于自己
    if (user.isSuperuser === 1 || user.isStaff === 1) {
      this.ctx.error(NO_PERMISSION_UPDATE_SYSTEM_USER_ERROR);
    }
    // 判断修改用户是否属于自己平台
