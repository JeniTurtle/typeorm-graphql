...
import { Menu, MenuIsCategory } from '@typeorm/account/entity/auth/menu';
...
export default class MenuService extends BaseService<Menu> {
  ...
  constructor(@InjectRepository(Menu, 'accountDB') readonly repository: Repository<Menu>) {
  ...
  async getMenusByUser(userId: string) {
    ...
    const ret = await this.repository
      ...
      .where('menu.platformId = :platformId')
      .andWhere('menu.deletedAt is null')
      .andWhere('menu.is_category = :isCategory', { isCategory: MenuIsCategory.NO })
  ...
  async getMenusByRole(roleId: string | string[]) {
    ...
    const ret = await this.repository
      .createQueryBuilder('menu')
      .where('menu.platformId = :platformId', { platformId })
      .andWhere('menu.deletedAt is null')
      .andWhere('menu.is_category = :isCategory', { isCategory: MenuIsCategory.NO })
  ...
  async addUserMenu({ userId, menuIds }: { userId: string; menuIds: string[] }, menuUserRepo?: Repository<MenuUser>) {
    ...
    // 新增用户没有的菜单
    const results = await menuUserService.createMany(data);
  ...
  async addRoleMenu({ roleId, menuIds }: { roleId: string; menuIds: string[] }, menuRoleRepo?: Repository<MenuRole>) {
    ...
    // 新增用户没有的菜单
    const results = await menuRoleService.createMany(data);
    return existingData.concat(results);
  }
  ...
  async delUserMenu(
    ...
    return await menuUserService.delete(where);
  }
  ...
  async delRoleMenu(
    ...
    return await menuRoleService.delete(where);
  }
