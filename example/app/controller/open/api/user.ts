...
import { Post, Controller, Summary, Parameters, Get, Put, Delete } from 'egg-shell-plus';
...
import { UserGender, UserIsActive } from '@typeorm/account/entity/auth/user';
...
const updateUserParams = {
  ...
  password: Joi.string()
    ...
    .description('密码'),
  gender: Joi.string()
    .valid(UserGender.MAN, UserGender.WOMAN, UserGender.UNKNOWN)
    .default(UserGender.UNKNOWN)
    .description('性别'),
  isActive: Joi.string()
    .valid(UserIsActive.YES, UserIsActive.NO)
...
export default class UserController {
  ...
  public async update(ctx: Context) {
    ...
    const ret = await this.userService.updateUser(body, {
      id: userId,
    });
    // 修改redis用户信息
    ret &&
  ...
  public async delete(ctx: Context) {
    ...
    const ret = await this.userService.deleteById(userId);
    // 删除redis用户信息
  ...
  public async updateSelf(ctx: Context) {
    const {
      request: { body },
      userData: { userinfo },
    } = ctx;
  ...
  @Get()
  @Summary('查看本人信息接口')
  public async selfDetail(ctx: Context) {
