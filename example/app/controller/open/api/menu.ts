...
const createMenuParams = Joi.object().keys({
  ...
  isCategory: Joi.number()
    .integer()
    .valid(MenuIsCategory.YES, MenuIsCategory.NO)
    .description('是否是分类，1是，0不是'),
  supreiorId: Joi.string().description('上级ID'),
});

@Controller('对外开放的菜单管理接口')
export default class MenuController {
  ...
  async list(ctx: Context) {
    ...
    // 获取上级菜单列表
    menus.rows = await this.menuService.relaSupreior(menus.rows);
    return menus;
  }
  ...
  async update(ctx: Context) {
    ...
    if (menu) {
      return await this.menuService.updateById(body, menuId);
    }
  }
  ...
  async delete(ctx: Context) {
    ...
    const menu = await this.menuService.checkMenu(menuId);
    if (menu) {
      return await this.menuService.deleteById(menuId);
    }
  }
  ...
  async addMenusWithUser(ctx: Context) {
  ...
  async deleteMenusWithUser(ctx: Context) {
    const { userId, menuIds } = ctx.request.body;
  ...
  async addMenusWithRole(ctx: Context) {
  ...
  @PermissionWithAction(['write_menu', 'write_role'])
  async deleteMenusWithRole(ctx: Context) {
    const { roleId, menuIds } = ctx.request.body;
  ...
  async transferMenusWithUser(ctx: Context) {
    const { userId, menuIds } = ctx.request.body;
    const [user, filteredIds] = await Promise.all([
      this.userService.checkUser(userId),
      this.menuService.filterOwnMenus(menuIds),
  ...
  async transferMenusWithRole(ctx: Context) {
    const { roleId, menuIds } = ctx.request.body;
    const [role, filteredIds] = await Promise.all([
      this.roleService.checkRole(roleId),
      this.menuService.filterOwnMenus(menuIds),
