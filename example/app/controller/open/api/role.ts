...
import { Post, Controller, Summary, Parameters, Get, Put, Delete, Description } from 'egg-shell-plus';
...
export default class RoleController {
  ...
  async update(ctx: Context) {
    ...
    return await this.roleService.updateById(body, roleId);
  }
  ...
  async addRolesWithUser(ctx: Context) {
  ...
  @PermissionWithAction(['write_user', 'write_role'])
  async deleteRolesWithUser(ctx: Context) {
    const { userId, roleIds } = ctx.request.body;
  ...
  @PermissionWithAction(['write_user', 'write_role'])
  async transferRolesWithUser(ctx: Context) {
    const { userId, roleIds } = ctx.request.body;
    const [user, filteredIds] = await Promise.all([
      this.userService.checkUser(userId),
      this.roleService.filterOwnRoles(roleIds),
