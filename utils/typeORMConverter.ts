...
function uniquesForEntity(entity: EntityMetadata): string[] {
  return entity.uniques.reduce<string[]>((arr, unique: UniqueMetadata) => {
...
export function generateEnumMapImports(generatedFolder: string): string[] {
  ...
  Object.keys(enumMap).forEach((tableName: string) => {
    Object.keys(enumMap[tableName]).forEach((columnName: string) => {
      ...
      const relativeFilePath = path.relative(generatedFolder, filename);
      imports.push(`import { ${enumColumn.name} } from '${relativeFilePath}'
`);
    });
  });
  ...
  Object.keys(classMap).forEach((tableName: string) => {
    ...
    imports.push(`import { ${classObj.name} } from '${relativeFilePath}'
`);
  });
...
export function entityToCreateInput(entity: EntityMetadata): string {
  ...
  entity.columns.forEach((column: ColumnMetadata) => {
    if (
      ...
      if (column.enum || column.type === 'json' || column.type === 'jsonb' || column.type === 'varying character') {
        fieldTemplates += `
          @TypeGraphQLField(() => ${graphQLDataType}, ${nullable})
  ...
  entity.columns.forEach((column: ColumnMetadata) => {
    ...
    if (column.isPrimary || graphqlType === GraphQLID) {
      ...
      fieldTemplates += `
        ...
        ${column.propertyName}_lte?: ${tsType};
      `;
    } else if (column.type !== 'json' && column.type !== 'jsonb' && column.type !== 'varying character') {
